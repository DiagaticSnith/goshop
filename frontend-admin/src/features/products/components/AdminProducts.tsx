...
export default function AdminProducts(){
  ...
  const handleCreateProduct = () => setIsShowCreateProduct(true);
      ...
      <div className="mt-4 p-5 bg-white drop-shadow-custom rounded-md">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center space-x-2">
            <button id="create-product" onClick={handleCreateProduct} className="flex items-center bg-primary text-white text-sm p-2 xs:px-6 xs:py-3 rounded-md transition hover:bg-opacity-90">
              <AiOutlinePlus className="inline-block mr-0 xs:mr-2 w-6 h-6" />
              <span className="sr-only xs:not-sr-only xs:inline">Create new</span>
            </button>
          </div>
        </div>
        <ProductsDashboard isAdmin={isAdmin} />
      </div>
      {isShowCreateProduct && (<CreateProductPopup setIsShowCreateProduct={setIsShowCreateProduct} />)}
    </>
  )
}
