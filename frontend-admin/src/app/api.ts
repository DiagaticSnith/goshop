...
const getDefaultBackend = () => {
  ...
  try {
    ...
    if (typeof window !== "undefined" && host && (host.startsWith("user-") || host.startsWith("admin-"))) {
      console.warn("[api] Forcing backend to https://backend-production-6e94.up.railway.app (temporary)");
      console.log("[api] Resolved baseURL -> https://backend-production-6e94.up.railway.app");
      return "https://backend-production-6e94.up.railway.app";
    }
    try {
